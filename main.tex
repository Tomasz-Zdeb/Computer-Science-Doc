...
% >>>>>>> DEFINECOLORS
% href colors
\definecolor{hrefurl}{RGB}{80,80,80}
\definecolor{hreflink}{RGB}{80,80,80}
\definecolor{cite}{RGB}{80,80,80}
...
{
name=pipeline,
description={a chain of processing elements (processes, threads, coroutines, functions, etc.), arranged so that the output of each element is the input of the next}
}

\newglossaryentry{glossary}
{
name=glossary,
description={an alphabetical list, with meanings, of the words or phrases in a text that are difficult to understand. Also a \LaTeX package that supports acronyms and multiple glossaries creation},
plural={glossaries} %custom plural form
}

\newglossaryentry{variable}
{
name=variable,
description={an abstract storage location paired with an associated symbolic name, which contains some known or unknown quantity of information referred to as a value}
}

\newglossaryentry{parameter}
{
name=parameter,
description={a special kind of \gls{variable} used in a subroutine to refer to one of the pieces of data provided as input to the subroutine.}
}

\newglossaryentry{bib}
{
name={.bib},
description={file extension used for \Gls{bibtex} source files, containing bibliography items}
}
...
\author{\LARGE Created and typeset by:\vspace{1em} \\ \LARGE Tomasz Zdeb and Contributors}
...
\input{content/LaTeX/LaTeX} %LaTeX Part

\input{content/windows/windows} %Windows Part

\input{content/linux/linux} %Linux Part

\input{content/general_concepts/general_concepts} %General Concepts Part
